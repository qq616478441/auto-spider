	...
	for i in range(0,len(data)):
		...
		if data[i].has_key('chinese_tag') and data[i]['tag_url']!='video' and data[i]['more_mode']==True:
			if data[i]['chinese_tag']!= None:
				...
				classify=urllib.urlencode({"classify":data[i]['chinese_tag']})
				#print type(classify)
			if data[i].has_key('title'):
				#print type(data[i]['title'])
				#title=data[i]['title'].decode('gbk','ignore').encode('utf-8','ignore')
				#print type(title)
				title=urllib.urlencode({"title":data[i]['title']})
				print type(title) 
				print title
			if data[i].has_key('source'):
				source=urllib.urlencode({'source':data[i]['source']})
				print source
			if data[i].has_key('abstract'):

				abstract=urllib.urlencode({'abstract':data[i]['abstract']})
			...
			if temp_url!=' ' and article_genre=='article':
				...
				if len(sel)!=0:
					for i in range(len(sel)):
						#print sel[i]
						news_origin+=sel[i]
					news_origin=urllib.urlencode({'news_origin':news_origin})
				...
				news_content=urllib.urlencode({'news_content':news_content.getContext()})
